...
// UDP socket setup and Kinect desired data parameters
...
/// <summary>
/// Parses config file, updates socket/data parameters, and initializes UDP
/// </summary>
/// <param name="configFile">string containing config file name</param>
/// <param name="envp">process environment (optional)</param>
/// <returns>status</returns>
int setupUDP(const char* configFile, char** envp)
{
	...
	tmp = config.pString("ip_local").c_str();
	strcpy_s(ip_local_scp, tmp.c_str());

	port_local_ss = (short)config.pInt("port_local");

	tmp = config.pString("ip_remote").c_str();
	strcpy_s(ip_remote_scp, tmp.c_str());
	
	port_remote_ss = (short)config.pInt("port_remote");
